-- SQL CORRIGIDO - Execute este script no SQL Editor do Supabase
...
-- ================================================================
-- 2. CRIAR/ATUALIZAR TABELA PASSWORD_RESET_TOKENS (SCHEMA PUBLIC)
-- ================================================================
...
CREATE INDEX IF NOT EXISTS password_reset_tokens_manager_id_idx ON public.password_reset_tokens(manager_id);
CREATE INDEX IF NOT EXISTS password_reset_tokens_employee_id_idx ON public.password_reset_tokens(employee_id);
...
DROP POLICY IF EXISTS "managers_own_reset_tokens" ON public.password_reset_tokens;
DROP POLICY IF EXISTS "employees_own_reset_tokens" ON public.password_reset_tokens;

-- Políticas de segurança (simplificadas - sem auth_user_id que não existe)
CREATE POLICY "managers_reset_tokens" ON public.password_reset_tokens
FOR ALL USING (manager_id IS NOT NULL);

CREATE POLICY "employees_reset_tokens" ON public.password_reset_tokens
